<?php
        $array = array();
        $matrix = array();
        $distance_matrix = json_decode($_POST["upper_matrix"]);
        $point_ids = json_decode($_POST["point_ids"]);
//dimiourgoume to upper matrix apo ton pinaka apostasewn
        for($i = 0 ; $i <sizeof($point_ids) ; $i++){
            for($j = 0 ; $j <sizeof($point_ids) ; $j++){
                if($i >= $j){
                    
                }
                else $array[$j] = $distance_matrix[$i][$j];
            }
            $matrix[$i] = $array;
            unset($array);
            $array = array();
        ...
        $DBSCAN = new DBSCAN($matrix, $point_ids);
        $epsilon = 30;
        $minpoints = 3;
...
class DBSCAN {
	...
	public function dbscan($epsilon, $min_points)  
	{
		...
		echo json_encode($this->clusters);
	}
}
?>